...
eval_match({var, Id}, Str, Env, Prgm) -> 
	...
	{Id, {lazy, Lazy, Closure}} ->
		...
		{ok, Evaluated} ->
		    ...
		    %% evaluated result. If we lookup the variable
		    %% again, we will have to evauate it ones more.
...
%%% This is new, we need to check if two datstructures are equal. We
...
eval_equal(Str, {lazy, Lazy, Closure}, Env, Prgm) ->
	...
	error ->
	    %% The equality test coudl also couse an error!
	    error
    end;
